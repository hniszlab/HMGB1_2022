...
# Droplet analysis from HMGB1 protein preps with (SEC cleaned preps - experiment 2)

library(dplyr)

obj <- read.csv2(file = "analysis_results_experiment2/20220506_SEC_clean_phase_diagram_2_Object.csv", stringsAsFactors = F)
...
write.table(print_table, file = "Experiment_2_mean_PSF_values.txt", sep = "\t",
            row.names = F, col.names = TRUE, quote = F)

# Print all values:
write.table(df_PSF, file = "Experiment_2_PSF_values.txt", sep = "\t",
            row.names = F, col.names = TRUE, quote = F)
