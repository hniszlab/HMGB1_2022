...
# Droplet analysis from HMGB1 protein preps with (SEC cleaned preps - experiment 1)

obj <- read.csv2(file = "analysis_results_experiment1/20220503_SEC_clean_phase_diagram_1_Object.csv", stringsAsFactors = F)
...
write.table(print_table, file = "Experiment_1_mean_PSF_values.txt", sep = "\t",
            row.names = F, col.names = TRUE, quote = F)

# Print all values:
write.table(df_PSF, file = "Experiment_1_PSF_values.txt", sep = "\t",
            row.names = F, col.names = TRUE, quote = F)
